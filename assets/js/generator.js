...
import { svg, getGroupColor, darkenColor, FONT_FAMILY } from "./config.js";
...
function createNodeElement({ n, nodeX, nodeY, groupColor, tag, order }) {
  ...
  if (n.link) {
    ...
    a.append(g);
    animation.register(a, order);
    return a;
  } else {
    animation.register(g, order);
    return g;
  }
}
...
function createGroupTitle({
  ...
  animation.register(titleEl, order);
  return titleEl;
}
...
export function createCustomCircles(
  ...
  order += rings.length * (showNumbers ? 2 : 1);

  const titleEl = createGroupTitle({
    title,
    desc,
    tag,
    groupColor,
